    ...
    req.getConnection((err, conn) => {
        conn.query('UPDATE reporte set estatus = ? WHERE id_reporte = ?', [newCustumer, id_reporte], (err, reportes) => {
            if (err) {
                ...
                const mensajeError = 'El reporte he sido cambiado de: Pendiente a En_Atencion';
                const script = `
                <script>
                    alert('${mensajeError}');
                    window.location.href = "/menumoderacion"; // Redirigir a la página de inicio de sesión
                </script>
                    `;
                res.send(script);
            }
                    ...
                    if (err) {
                        ...
                        conn.query('UPDATE ciudadano set n_penalizaciones = 1 WHERE id_ciudadano = ?', [userrep], (err, ban) => {
                            if (err) {
                                ...
                                const mensajeError = 'El reporte ha sido penalizado';
                                const script = `
                            <script>
                                alert('${mensajeError}');
                                window.location.href = "/menumoderacion"; // Redirigir a la página de inicio de sesión
                    ...
                    if (err) {
                        ...
                        conn.query('UPDATE ciudadano set n_penalizaciones = 1 WHERE id_ciudadano = ?', [userrep], (err, ban) => {
                            if (err) {
                                ...
                                const mensajeError = 'El reporte ha sido eliminado';
                                const script = `
                                <script>
                                    alert('${mensajeError}');
                                    window.location.href = "/menumoderacion"; // Redirigir a la página de inicio de sesión
